...
function PreprocessLog(){
    ...
    if [ "${panicenable}" = "${argtrue}" ] || [ x"${camerapanic}" = x"true" ]; then
        ...
        if [ x"${decrypt}" != x"true" ]; then
            ...
            mv /data/oppo_log/unboot_* ${ROOT_SDCARD_LOG_PATH}/
...
function transferSystrace(){
    ...
    if [ -d "${systrace_path}" ]; then
        mkdir -p ${newpath}/systrace
        echo "$(date +%F-%H:%M:%S) tranfer2SDCard:copy systrace to ${newpath}/systrace " >> ${SDCARD_LOG_BASE_PATH}/tranfer2.info
        mv ${systrace_path}/* ${newpath}/systrace
    fi
}
...
function tranferWifi(){
    #P wifi log
    mkdir -p ${newpath}/vendor_logs/wifi
    chmod 770 /data/oppo/log/temp/*
    cp -r /data/oppo/log/temp/* ${newpath}/vendor_logs/wifi
    rm -rf data/oppo/log/temp/*
}
...
function LogcatKernel2(){
    ...
    then
    /system/system_ext/xbin/klogd -f - -n -x -l 7 | tee - ${ROOT_SDCARD_kernel_LOG_PATH}/kinfo0.txt | awk 'NR%400==0'
    fi
}
function tcpdumpLog2(){
    ...
    then
       ...
       #else
       tcpdump -i any -p -s 0 -W ${tcpdumpCount} -C ${tcpdumpSize} -w ${ROOT_SDCARD_netlog_LOG_PATH}/tcpdump
       #endif
    fi
}
...
function getSystemStatus() {
    ...
    then
        ...
        echo "$(date +%F-%H:%M:%S) dumpSystem:dropbox" >> ${SDCARD_LOG_BASE_PATH}/tranfer2.info
